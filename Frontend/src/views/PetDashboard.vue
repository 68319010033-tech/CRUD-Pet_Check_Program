...
const handleLogout = async () => {
  try {
    ...
    showNotification('ออกจากระบบไม่สำเร็จ: ' + err.message, 'error');
  }
};
</script>
    ...
    <nav class="sticky top-0 z-40 bg-[#FAF7F2]/95 backdrop-blur-md border-b border-[#F3EDE2]/60 px-6 py-4 md:px-12 flex justify-between items-center">
      ...
      <div class="flex items-center space-x-3">
        <div class="hidden sm:flex items-center space-x-2 bg-[#F3EDE2]/40 px-3.5 py-1.5 rounded-full text-xs font-semibold">
          ...
          <span class="text-[#2A2A2A]/80">{{ currentUser?.email || apiBaseUrl }}</span>
        </div>
        <button 
          @click="openAddModal" 
        ...
        <button
          @click="handleLogout"
          class="bg-[#F3EDE2]/60 hover:bg-[#F3EDE2] text-[#2A2A2A]/80 px-4 py-2.5 rounded-full text-sm font-semibold transition-all duration-300"
        >
          ออกจากระบบ
        </button>
      </div>
